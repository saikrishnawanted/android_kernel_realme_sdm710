...
#ifdef VENDOR_EDIT//OuYangBaiLi@BSP.CHG.Basic 2019/03/05 modify for factory otg
...
static int oppo_get_otg_online_status(void)
{
	...
	#ifdef VENDOR_EDIT
	//OuYangBaiLi@BSP.CHG.Basic 2019/01/23 modify for otg
	if(oppo_get_otg_switch_status())
...
void oppo_set_otg_switch_status(bool value)
{
	...
	chg = &chip->pmic_spmi.smb2_chip->chg;
#ifdef VENDOR_EDIT//OuYangBaiLi@BSP.CHG.Basic 2019/03/05 modify for factory otg
	if((qpnp_get_prop_charger_voltage_now()> 4000) && (chg->real_charger_type == 5)){
		chip->otg_switch = false;
		otg_count = true;
		pr_err("%s: usbin when otg_switch charger_type = %d \n", __FUNCTION__,chg->real_charger_type);
		return;
		}
#endif /* VENDOR_EDIT */
...
#ifdef VENDOR_EDIT
//OuYangBaiLi@BSP.CHG.Basic 2019/01/23 modify for otg
bool otg_irq_stat = false;
#endif /* VENDOR_EDIT */
void otg_enable_id_value (void)
{
    /*Jun.Wei@RM.BSP.CHG.Basic, 2018/12/21, add for OTG id_pin control*/
...
#ifdef VENDOR_EDIT
/* OuYangBaiLi@BSP.CHG.Basic, 2019/01/08,Add for 18621 RF interfere */
		chg->chg_freq.freq_5V		= 800;
